...
export const PreparationView: React.FC<PreparationViewProps> = ({
    ...
    reader.onload = (event) => {
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          const list = Array.isArray(parsed) ? parsed : parsed.pedidos || [];
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-6 space-y-8 animate-fadeIn">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <button
              ...
              className="text-xs font-semibold text-slate-600 hover:text-slate-900 flex items-center gap-1.5 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-sm transition"
            >
            ...
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">
              {trips.length} Viagens Alocadas na Carroceria Aberta
            </span>
          </div>
          ...
          <div className="space-y-4">
            {trips.map((viagem, index) => {
              const has6m = viagem.pedidos_carroceria.some((p) => p.possui_itens_6m);
              return (
                <div
                  ...
                  <div className="space-y-1 w-full md:w-auto">
                    <div className="flex items-center gap-2.5">
                      <h3 className="text-base md:text-lg font-bold text-slate-900">
                        Caminhão {index + 1} - {viagem.eixo_nome}
                      </h3>
                      {has6m && (
      ...
      {/* =================================================================== */}
      {/* ETAPA 3: Detalhe do Caminhão / Eixo (Figma Imagem 4)                */}
      {/* =================================================================== */}
      {currentStep === 'detail' && selectedTripCarga && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Voltar para Lista de Caminhões</span>
            </button>
                          ...
                          <td colSpan={6} className="py-2.5 px-4">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                              ...
                              <span className="text-[11px] font-mono text-slate-700">
                                Posição Carroceria: {ped.posicao_carroceria} |{' '}
                        ...
                        {ped.itens.map((it, itIdx) => (
                          ...
                          >
                            <td className="py-2.5 px-4 font-medium text-slate-900">
                              {it.descricao}
                              <span className="block text-[10px] text-slate-400 font-mono">
                                Cód: {it.codigo} {it.e_item_6m && '• ⚠️ PEÇA LINEAR DE 6M'}
                              </span>
                            </td>
                            <td className="py-2.5 px-4 text-slate-600">{it.unidade}</td>
                            <td className="py-2.5 px-4 text-center font-mono font-semibold">
